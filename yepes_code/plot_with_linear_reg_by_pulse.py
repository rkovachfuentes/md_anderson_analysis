...
def plot_a_vs_b(log_file, a, b, filter_var, filter_val, group_var, savePlot = False, no_3 = False):
    ...
    print(log_df['Z'].unique())
    # log_df = log_df[[a, b, filter_var, group_var]]
    log_df = log_df[log_df[filter_var] == filter_val]
    log_df = log_df[log_df["Pulse"] > 0.1]
    # log_df = log_df[log_df["Z"] > 30]
    print(log_df.head())
    color_dict = {"0.1":"red","0.5":"black","1.0":"blue","2.0":"green","3.0":"red"}
    ...
    for group_tuple, val in grouped_mean_multi:
        if no_3 and (str(group_tuple[0]) == "3.0"):
            ...
            if group_tuple[0] == initial_grp:
                ...
                print(z_labels)
                coeffs = np.polyfit(grp_a, grp_means, 1)
                p = np.poly1d(coeffs)
                plt.plot(grp_a, p(grp_a), color=color_dict[str(initial_grp)], linestyle='dashed')
                grp_means = []
                grp_a = []
                grp_errs = []
                z_labels = []
    ...
    plt.plot(grp_a, p(grp_a), color=color_dict[str(initial_grp)], linestyle='dashed')
    plt.text(
            0.55 * max(log_df[a]),  # X-coordinate (adjust based on your plot range)
            min(log_df[b])+0.1 * (max(log_df[b])-min(log_df[b])),  # Y-coordinate (adjust based on your plot range)
            f"Date: 07/30/25\nBeam: 85V\nHV: 20V\nZ: various, averaged \nacross all values",
            fontsize=10,
            color="purple",
            bbox=dict(facecolor='white', alpha=0.7, edgecolor='purple')  # Optional styling
    )
    ax.set_title(f"Dose vs Mean Area (BNL Diode), Grouped By {group_var}, HV={filter_val}")
    ax.set_xlabel(f"dose (C*10^-8)")
