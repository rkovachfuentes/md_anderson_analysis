...
def get_area_old(file_path, range_dict, date, pulse=2, Z=60, HV=0, beam="Electrons 85V", ifile=1560, selected_channel="CH1", show_saturation_correction=False):
    npeaks_list = []
    corresponding_files = []
    pulse_widths = []
    ...
    if pulse >= 1:
        ...
        shifted_end_index = oct_file_generator.straight_line_across(time, signal, shifted_start_index, signal_range)[0]
        '''if ((shifted_end_index - shifted_start_index) <= 100):
            too_narrow = True
            counter = 1
            while too_narrow:
                new_end = oct_file_generator.straight_line_across(time, signal, shifted_end_index, signal_range)
        ...
        shifted_end_index = oct_file_generator.straight_line_across(time, signal, shifted_start_index, signal_range)
    ...
    if savePlot:
        ...
        # plt.plot(time_signals, baseline_signals, 'black', label="selected to average over")  # high frequency noise removed
...
def get_area(file_path, range_dict, date, pulse=2, Z=60, HV=0, beam="Electrons 85V", ifile=1560, selected_channel="CH1", show_saturation_correction=False):
    ...
    baseline_signals = signal[300:remove_start_index] + signal[remove_end_index:-300]
    baseline_avg = np.median(baseline_signals)
    ...
    print("shapes of signal and time")
    print(np.shape(corrected_signal_removed_values),np.shape(removed_time))
    signal_area   = abs(simpson(y=corrected_signal_removed_values, x=removed_time))
    w = savgol_filter(signal, 200, 2)
...
def signal_region_finder_new(file_path, range_dict, date, pulse=2, Z=60, HV=0, beam="Electrons 85V", ifile=1560, selected_channel="CH1", show_saturation_correction=False):
    npeaks_list = []
    corresponding_files = []
    pulse_widths = []
    time, signal = oct_file_generator.read_csv(file_path, selected_channel)
