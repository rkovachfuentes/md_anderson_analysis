...
def straight_line_across_old(time_data, channel_data, drop_idx, signal_range):
...
def get_area_old(file_path, range_dict, date, pulse=2, Z=60, HV=0, beam="Electrons 85V", ifile=1560, selected_channel="CH1", show_saturation_correction=False):
    ...
    shift_points = int(0.2e-6 / time_step)  # Number of points to shift earlier
    shifted_start_index = max(0, start_index)  # Ensure index is not negative  - shift_points
    shifted_start_index += 103
    if pulse >= 1:
        print("pulse over 1")
        print("shifted start index") 
        print(shifted_start_index)
        shifted_end_index, time, signal = straight_line_across(time, signal, shifted_start_index, signal_range)
        print(shifted_end_index)
        print(shifted_start_index)
        if ((shifted_end_index - shifted_start_index) <= 100):
            too_narrow = True
            counter = 1
...
if __name__ == "__main__":
    get_area("/Users/rkfuentes/Documents/md_anderson_analysis/yepes_code/data/2025-11-20/scope-results-2025-11-20-0147.csv", range_dict, "2025-11-20", pulse=0.5, Z=50, HV=100, beam="Electrons 191V", ifile=1560, selected_channel="CH1", show_saturation_correction=False)
    get_area("/Users/rkfuentes/Documents/md_anderson_analysis/yepes_code/data/2025-11-20/scope-results-2025-11-20-0148.csv", range_dict, "2025-11-20", pulse=0.5, Z=50, HV=100, beam="Electrons 191V", ifile=1560, selected_channel="CH1", show_saturation_correction=False)
